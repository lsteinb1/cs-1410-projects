...
public class HowMany {
  ...
  /**
   * Find how many instances of a string are in a body of text.
   * USE NO STRING METHODS OTHER THAN equalsIgnoreCase & equals.
   * This challenge is also a coding gold, meaning whoever uses the least amount of lines (semicolons)
   * will win the challenge.
   */
  public static int howMany(String instance, String bodyOfText) {
	  String[] strArr = bodyOfText.split(" ");
	  int count = 0;
	  
	  for (String word : strArr) {
		  if(word.equalsIgnoreCase(instance)) {
			  count++;
		  }
	  }
	  
	  return count; // TODO
	  // you said nothing of StringBuilder methods....
  }
  ...
  public static void main(String[] args) {
    test("The", "the", 446);
    test("The Caps", "The", 446);
    test("Bee", "bee", 173);
    test("Bee Caps", "Bee", 173);
    test("Barry", "barry", 50);
    test("Barry Caps", "Barry", 50);
    test("Period", ".", 48111);
  }

  public static void test(String name, String input, int expected) {
    final int real = howMany(input, BODY_OF_TEXT);
    if (real == expected) {
  ...
  final static String BODY_OF_TEXT = """
